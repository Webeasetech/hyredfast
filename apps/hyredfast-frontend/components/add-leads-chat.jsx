        ...
        <div className="max-h-[200px] overflow-y-auto divide-y divide-gray-50">
          {leads.map((lead, i) => (
            <div key={i} className="px-3 py-2 flex items-start gap-2">
              <UserPlusIcon className="w-3.5 h-3.5 text-muted-foreground mt-0.5 shrink-0" />
                  ...
                  Object.keys(lead.personalization).length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1">
                      {Object.entries(lead.personalization).map(([k, v]) => (
                        <span
                          key={k}
                          className="text-[10px] bg-accent text-muted-foreground px-1.5 py-0.5"
                        >
                          {k}: {v}
                        </span>
                      ))}
  ...
  return (
    <Flipper flipKey={isOpen} spring={{ stiffness: 600, damping: 60 }}>
      ...
      {isOpen && (
        ...
        >
          <div className="fixed bottom-6 right-6 z-50 w-[400px] max-h-[560px] bg-white border border-border flex flex-col overflow-hidden rounded-lg">
            <div
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                ...
                {messages.length === 0 && !parsedLeads && !activeRun && (
                  <div className="text-center py-6">
                    <MessagePlusIcon className="w-8 h-8 text-muted-foreground mx-auto mb-2" />
