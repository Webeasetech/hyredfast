...
/**
 * The grid for one job application's leads.
 *
 * Layout only: which columns, in which order, in which rows. What a cell does
 * lives in `composer-cells`, what counts as an error lives in `lib/lead-draft`,
 * and moving between cells lives in `lib/sheet-navigation` — this places them.
 */
export default function ComposerGrid({
  ...
  fixedValues = {},
  states,
  /** rowId -> { column: message }. Empty until a publish has been attempted:
      typing a lead in should not be narrated as a series of failures. */
  errors = {},
  selected,
  /** { rowId, column } of the cell holding focus, for the row and column tint. */
  active,
  widthOf,
  onResizeColumn,
  onToggleRow,
  ...
  // Widths are shared across every group, so the blocks read as one sheet.
  const gridStyle = {
    gridTemplateColumns: `2rem 2.5rem ${columns
      .map((col) => `${widthOf(col)}px`)
      .join(" ")} 2.25rem`,
  };
  ...
  return (
    <div className="w-max min-w-full" onPaste={onPaste}>
      {/* Column headers. Sticky under the group's own header, which is sticky
          at the top of the scroll area — so on a long sheet you always know
          both which job application you are in and which column you are on. */}
      <div
        className="sticky top-12 z-20 grid items-center border-b border-border bg-muted"
        style={gridStyle}
      >
      ...
      {rows.map((row, index) => {
        ...
        const isActive = active?.rowId === row.id;
        ...
        return (
          ...
          >
            ...
            <span
              className={cn(
                "flex h-8 items-center justify-center border-r border-border text-xs tabular-nums",
                isActive
            ...
            {columns.map((col) =>
              ...
              col in fixedValues ? (
                ...
                <EditableCell
                  ...
                  onMove={(step) => onMove({ rowId: row.id, column: col, ...step })}
